...
async def free_log(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    goal = goal_match.group(1).strip() if goal_match else text.strip()
...
async def today_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    conn.close()
    if not rows:
        await update.message.reply_text("No study logged today.")
        return
...
def main():
    ...
    # Midnight summary at 18:30 UTC = 00:00 IST
